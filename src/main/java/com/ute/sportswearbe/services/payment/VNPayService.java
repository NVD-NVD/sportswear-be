...
import com.ute.sportswearbe.utils.PaymentProcessUtlis;
...
public class VNPayService implements PaymentService{
    ...
    @Override
    public PaymentResp createPayment(String orderID, String url_return, Principal principal) throws IOException {
        User user = userService.getUserByPrincipal(principal);
        Order order = orderService.getOrderById(orderID);

        double amount = order.getTotal().getPrice() * 100;
...
//        String bank_code = paymentDto.getBankCode();
        ...
        vnp_Params.put("vnp_ReturnUrl", url_return);
    ...
    public TransactionStatusDto transactionHandle(String amount, String bankCode, String bankTranNo,
        ...
        Transaction transaction = createTransaction(amount, bankCode, bankTranNo,
                                        cardType, orderInfo,payDate, responseCode, tmnCode, transactionNo,
                                        txnRef,secureHashType,secureHash);
        order.setProcessing(PaymentProcessUtlis.Da_Thanh_Toan.name());
